...
public class PeriodSearchControllersTests {
    ...
    class search {
        ...
        void givenInitialAccess_whenSearch_thenStatus200AndReturnPeriodSearchViewWithoutErrors() throws Exception {
        ...
        void givenInvalidFormat_whenSearch_thenHasTypeMismatchErrorAndReturnFormView() throws Exception {
            mockMvc.perform(get("/validation/period/search")
                    ...
                    .andExpect(model().attributeHasFieldErrorCode("periodSearchForm", "startDate", "typeMismatch"))
                    .andExpect(model().attributeHasFieldErrorCode("periodSearchForm", "endDate", "typeMismatch"));
        }
        ...
        void givenReversePeriod_whenSearch_thenHasChronologicalPeriodErrorAndReturnFormView() throws Exception {
            MvcResult result = mockMvc
                    .perform(get("/validation/period/search").param("startDate", "2026-01-01").param("endDate",
                            "2025-12-31"))
                    .andExpect(status().isOk())
                    .andExpect(view().name("validation/period-search-form"))
                    .andExpect(model().hasErrors())
